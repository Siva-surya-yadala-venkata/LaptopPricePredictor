...
overhyped_data = pd.read_csv('overhyped_laptops.csv')

# Feature Engineering: Apply the same preprocessing as before
overhyped_data['Inches'] = pd.to_numeric(overhyped_data['Inches'], errors='coerce')
overhyped_data['Weight'] = overhyped_data['Weight'].str.replace('kg', '').astype(float)
overhyped_data['Ram'] = overhyped_data['Ram'].str.replace('GB', '').astype(int)
overhyped_data['Price'] = pd.to_numeric(overhyped_data['Price'], errors='coerce')
overhyped_data['Touchscreen'] = overhyped_data['ScreenResolution'].apply(lambda x: 1 if 'Touchscreen' in x else 0)
overhyped_data['IPS'] = overhyped_data['ScreenResolution'].apply(lambda x: 1 if 'IPS' in x else 0)

# Convert screen resolution into total pixels and calculate PPI
overhyped_data['Resolution'] = overhyped_data['ScreenResolution'].str.extract(r'(\d+x\d+)')
overhyped_data[['Width', 'Height']] = overhyped_data['Resolution'].str.split('x', expand=True).astype(float)
overhyped_data['PPI'] = ((overhyped_data['Width'] ** 2 + overhyped_data['Height'] ** 2) ** 0.5) / overhyped_data['Inches']

# Convert categorical columns to category type and then encode them as integers
original_company_overhyped = overhyped_data['Company'].astype('category')
original_type_overhyped = overhyped_data['TypeName'].astype('category')
original_os_overhyped = overhyped_data['OpSys'].astype('category')

overhyped_data['Company'] = original_company_overhyped.cat.codes
overhyped_data['TypeName'] = original_type_overhyped.cat.codes
overhyped_data['OpSys'] = original_os_overhyped.cat.codes

# Define features (X) for the new dataset
X_overhyped = overhyped_data[['Inches', 'Weight', 'Company', 'TypeName', 'OpSys', 'Ram', 'Touchscreen', 'IPS', 'PPI']].values
y_overhyped = overhyped_data['Price'].values

# Predict the prices using the previously trained model
predicted_prices_overhyped = model.predict(X_overhyped)

# Plot a bar graph comparing actual prices and predicted prices
plt.figure(figsize=(12, 6))

# Indices for the laptops
indices = np.arange(len(overhyped_data))

# Bar plot
plt.bar(indices - 0.2, y_overhyped, width=0.4, label='Actual Prices', color='b', align='center')
plt.bar(indices + 0.2, predicted_prices_overhyped, width=0.4, label='Predicted Prices', color='r', align='center')

# Adding labels and title
plt.xlabel('Laptop Index')
plt.ylabel('Price')
plt.title('Actual vs Predicted Prices for Laptops from Overhyped Dataset')
plt.xticks(indices, overhyped_data['Company'].astype(str) + ' ' + overhyped_data['TypeName'].astype(str), rotation=90)
plt.legend()
