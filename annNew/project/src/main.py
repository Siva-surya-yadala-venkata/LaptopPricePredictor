...
def main():
    ...
    # Now read the overhyped_laptops.csv file
    try:
        overhyped_data = pd.read_csv("overhyped_laptops.csv")
        print("Available columns in overhyped_laptops.csv:", list(overhyped_data.columns))
    except FileNotFoundError:
        print("Error: overhyped_laptops.csv not found!")
        return
    except Exception as e:
        print(f"Error loading overhyped_laptops.csv: {str(e)}")
        return

    # Extract features for prediction (drop non-numerical columns and preprocess)
    overhyped_features = overhyped_data[['Inches', 'Weight', 'Cpu', 'Ram', 'Memory']]  # Adjust if necessary
    overhyped_features = overhyped_features.apply(pd.to_numeric, errors='coerce')  # Convert to numeric if possible

    # Ensure there are no missing values in the features for prediction
    overhyped_features = overhyped_features.fillna(overhyped_features.mean())

    # Normalize features the same way as training data (using the same scaler)
    overhyped_features_scaled = (overhyped_features - X_train.mean()) / X_train.std()

    # Use the trained model to predict prices for the new data
    predicted_prices_scaled = mlp_model.predict(overhyped_features_scaled)
    predicted_prices = predicted_prices_scaled * y_train_std + y_train_mean

    # Create a DataFrame with the actual and predicted prices
    comparison_overhyped_df = pd.DataFrame({
        'Laptop': overhyped_data['Company'] + ' ' + overhyped_data['TypeName'],
        'Actual Price': overhyped_data['Price'],
        'Predicted Price': predicted_prices.flatten()
    })

    # Plot bar graph comparing Actual and Predicted Prices
    plt.figure(figsize=(14, 8))
    comparison_overhyped_df.set_index('Laptop')[['Actual Price', 'Predicted Price']].plot(kind='bar', figsize=(12, 6))
    plt.title("Comparison of Actual and Predicted Prices for Laptops")
    ...
    comparison_overhyped_df.to_csv("overhyped_laptops_comparison.csv", index=False)
    print("\nActual vs Predicted prices for overhyped laptops saved to overhyped_laptops_comparison.csv.")
